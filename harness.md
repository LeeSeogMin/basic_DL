...
`docs/ch{N}.md`의 구조 규격은 `.ai/chapter-spec.md` §3에 있다. 게이트에서 확인하는 것은 아래 다섯 가지다.

```text
# 제N장 제목 — 부제
## [전반부] 이론 강의      (학습 목표 + 이론 절 5~7개 + 외부 시각 자료 절)
## [후반부] 실습           (실습 두 개. 각각 실행 결과·결과 읽기·해 보기)
### 책임 있는 AI 체크      (제출 전 점검 표)
### 과제 (LMS 제출)        (제출물 / 평가 기준 / 마감)
### 핵심 정리              (개념 한 줄 정리 표)
```

본문의 모든 수치는 `practice/chapter{N}/results/*.log`에서만 인용하고, 인용 뒤에 `_출처: ...log_`를 붙인다.

8주차와 15주차는 교재 장이 아니므로 이 게이트를 적용하지 않는다.
