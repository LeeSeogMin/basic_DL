...
이 장의 위험은 훈련 성적만 보고 판단하는 것이다.

**표 7.7** 제출 전 점검

| 점검 항목 | 확인 질문 |
|---|---|
| 두 성적 함께 | 훈련 정확도와 검증·테스트 정확도를 함께 적었는가 |
| 훈련 성능 과장 | 훈련 정확도가 1.000이라는 이유로 좋다고 적지 않았는가 |
| 과적합 판단 | 두 값의 차이가 클 때 과적합으로 읽었는가 |
| 복잡도 맹신 | 모델을 복잡하게 할수록 낫다고 적지 않았는가 |
| 테스트 재사용 | 테스트 데이터를 여러 번 보며 값을 맞추지 않았는가 |
| AI 제안 검토 | AI가 낸 해결책이 이 실습 조건에서 실제로 되는지 따졌는가 |
