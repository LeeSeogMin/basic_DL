...
직접 만든 데이터를 다루는 주다. 데이터가 어디서 왔고 무엇을 고쳤는지 남긴다.

**표 3.6** 제출 전 점검

| 점검 항목 | 확인 질문 |
|---|---|
| 개인정보 | 표에 실제 이름·학번·연락처가 들어가지 않았는가 |
| 합성 표시 | 만든 데이터가 합성이라는 것을 파일과 출력에 적었는가 |
| 결측 처리 | 결측을 0으로 채우지 않고, 어떻게 채웠는지 적었는가 |
| 파일 입출력 | 코드가 어떤 파일을 읽고 쓰는지 설명할 수 있는가 |
| AI 코드 검증 | AI가 만든 코드를 직접 실행해 결과를 확인했는가 |
