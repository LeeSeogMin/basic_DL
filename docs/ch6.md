...
학습을 일부러 망가뜨리는 주다. 망가진 결과를 지우지 않는 것이 핵심이다.

**표 6.4** 제출 전 점검

| 점검 항목 | 확인 질문 |
|---|---|
| 설정 변경 | AI가 학습률·epoch를 말없이 바꾸지 않았는가 |
| 실패 보존 | 발산해서 `inf`가 된 결과를 지우지 않고 그대로 기록했는가 |
| 수정 이유 | AI가 코드를 고쳤다면 왜 그렇게 고쳤는지 설명했는가 |
| 과장 표현 | "최적의 학습률" 같은 단정을 그대로 옮기지 않았는가 |
| 실행 확인 | 고친 코드를 직접 실행해 손실 곡선을 봤는가 |
