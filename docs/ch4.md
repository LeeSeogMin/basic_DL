...
| 용어 | 뜻 |
|---|---|
| 규칙 기반 방식 | 사람이 판단 규칙을 직접 작성하고 컴퓨터가 적용하는 방식 |
| 머신러닝 방식 | 데이터에서 판단 기준을 모델이 찾는 방식 |
| 분류(classification) | 정답이 범주(이름)인 문제 |
| 회귀(regression) | 정답이 숫자인 문제 |
| 훈련 데이터 | 모델이 배우는 연습문제 |
| 테스트 데이터 | 모델을 평가하는 처음 보는 시험문제 |
| 기준선 모델(baseline) | 새 모델이 최소한 넘어야 할 비교 기준 |
| 정확도(accuracy) | 전체 예측 중 맞힌 비율 |
| 혼동행렬(confusion matrix) | 모델이 어떤 정답을 어떤 예측으로 헷갈렸는지 보여주는 표 |
...
이번 실습의 목표는 2차원 점 분류기를 직접 실행하고, 모델이 틀린 사례를 분석하고, AI에게 해석을 맡긴 뒤 그 해석을 검증하는 것이다.

학생은 이번 실습 후 다음을 할 수 있어야 한다.

1. 2차원 점 분류기를 실행하고 결과 파일을 확인한다.
2. 판단 경계 그림과 혼동행렬 그림을 보고 해석한다.
3. 모델이 틀린 사례를 숨기지 않고 분석한다.
4. AI에게 결과 해석을 맡기고, AI 답변이 실행 결과와 맞는지 검증한다.
...
- 정확도 0.917은 기준선 0.521보다 높다. 모델이 점의 위치를 보고 판단한 결과다.
- 그러나 4개를 틀렸고, 그중 B그룹을 A그룹으로 틀린 경우가 3개다.
- 판단 경계 근처의 점은 두 그룹이 섞여 있어 모델이 틀릴 가능성이 높다.
- 정확도만 보고 "좋은 모델"이라고 단정하면 오류의 방향을 놓친다.
